...
* **MVP providers (prefer no pay, no card on file):**
  1. Primary: Groq (`openai/gpt-oss-120b`) for best free quality and speed
...
* Rough admin estimate with current console caps and a ~12k tokens per analysis guess: Groq ~16, Cerebras ~83, Gemini Flash Lite ~125 (shared across all users; model and quota dependent).
